...
| Candidate governance state | Memory Candidates and Promotion / Supersession records | Queue views, triage scores, contradiction reports |
...
2. Canonical writes persist into the appropriate memory domain: curated knowledge, procedures, operational work records, profile records, or governance state.
3. Deterministic extraction builds structural manifests from canonical sources.
4. Derived processes build context maps, atlases, embeddings, indexes, and orientation artifacts from canonical state and manifests.
5. Retrieval and orchestration use canonical state first and consult derived artifacts to narrow, explain, and rank candidate sources.
6. Governance outcomes either promote reviewed claims into canonical memory or keep them isolated as rejected or superseded candidates.
...
2. Preserve the source evidence needed for provenance.
3. Write directly to canonical memory when the signal is already an authoritative update for that domain.
4. Write to governance state when the signal is inferred, ambiguous, contradictory, or not yet strong enough to become canonical truth.
5. Refresh manifests, maps, indexes, and other derived artifacts from the updated canonical state.
6. Preserve promotion or supersession outcomes as explicit records rather than silent mutations.
