...
export async function supersedeMemoryCandidateEntry(
  ...
  const supersededCandidate = await engine.getMemoryCandidateEntry(input.superseded_candidate_id);
  ...
  const replacementCandidate = await engine.getMemoryCandidateEntry(input.replacement_candidate_id);
  ...
  const supersessionEntry = await engine.supersedeMemoryCandidateEntry({
  ...
  const updatedCandidate = await engine.getMemoryCandidateEntry(supersededCandidate.id);
  if (!updatedCandidate) {
