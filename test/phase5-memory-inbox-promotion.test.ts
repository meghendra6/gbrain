...
describe('phase5 memory-inbox-promotion benchmark', () => {
  test('--json prints a phase5 memory-inbox-promotion benchmark report shape', () => {
    const proc = spawnSync(['bun', 'run', 'scripts/bench/phase5-memory-inbox-promotion.ts', '--json'], {
      stdout: 'pipe',
      stderr: 'pipe',
    });
    ...
    expect(payload.workloads.map((workload: any) => workload.name)).toEqual([
      'memory_inbox_promotion_correctness',
