...
describe('phase5 memory-inbox-supersession benchmark', () => {
  test('--json prints a phase5 memory-inbox-supersession benchmark report shape', () => {
    const proc = spawnSync(['bun', 'run', 'scripts/bench/phase5-memory-inbox-supersession.ts', '--json'], {
      stdout: 'pipe',
      stderr: 'pipe',
    });
    ...
    expect(payload.workloads.map((workload: any) => workload.name)).toEqual([
      'memory_inbox_supersession_correctness',
